  ...
  - path: /root/k8s-join.sh
    ...
    content: |
      ...
      for i in $(seq 1 40); do
        ...
        echo "[worker-gpu] Tentative $i/40 — master pas encore prêt, attente 15s..."
        sleep 15
      done

      if [ -z "$JOIN_CMD" ]; then
        echo "[worker-gpu] ERREUR : impossible de récupérer le join-command depuis $MASTER_IP" >&2
        exit 1
      fi
...
final_message: |
  =====================================================
  NixOS kubeadm worker GPU ready — boot took $UPTIME seconds
  GPU      : rocm-smi / rocminfo
  Bootstrap: /root/k8s-join-bootstrap.log
